...
import telegramplus.SolarCalendar;
...
//                currentInfo = languagesDict.get(systemDefaultLocale.getLanguage());
    ...
    public static String formatDateChat(long date) {
        try {
            ...
            if (ApplicationLoader.applicationContext.getSharedPreferences("mainconfig", 0).getBoolean("persian_date", false)) {
                if (year == dateYear) {
    ...
    public static String formatDateOnline(long date) {
        try {
            ...
            if (dateDay == day && year == dateYear) {
                ...
                String format = formatString("formatDateAtTime", R.string.formatDateAtTime, getInstance().formatterMonth.format(new Date(1000 * date)), getInstance().formatterDay.format(new Date(1000 * date)));
                ...
                String format = formatString("formatDateAtTime", R.string.formatDateAtTime, getInstance().formatterYear.format(new Date(1000 * date)), getInstance().formatterDay.format(new Date(1000 * date)));
                return String.format("%s %s", new Object[]{getString("LastSeenDate", R.string.LastSeenDate), format});
            }
        } catch (Throwable e) {
            FileLog.e("tmessages", e);
